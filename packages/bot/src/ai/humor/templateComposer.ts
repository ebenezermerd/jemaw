/**
 * Template-only composer for Phase 1 dry pilot.
...
export function composeFromTemplates(
  ...
  if (text.includes("{{")) {
    // Unknown placeholder left — reject for fact safety.
    return null;
  }
  return { text, templateId: pick.id, style: pick.style };
}
...
export function renderPlaceholders(
  ...
  return body.replace(/\{\{(\w+)\}\}/g, (_, key: string) => {
    ...
    // Unknown or disallowed placeholder remains so composeFromTemplates rejects.
    return `{{${key}}}`;
  });
}
